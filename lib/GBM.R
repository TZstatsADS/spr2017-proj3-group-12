...
TrainGBM = function(feature_filename,labels_filename, param_trees = 500, param_shrinkage = 0.1, full_feature = FALSE)
{
  ...
  cat("Elapsed training time for  GBM with ", param_trees, "trees and shrinkage 0.1 is " , train_time, " seconds \n")
  cat("Validation error for GBM is ", 1 - tune_gbm$results$Accuracy)
...
TestGBM = function(gbm_model, feature_filename, param_trees = 500, full_feature = FALSE)
{
  t = proc.time()
  image_features = t(read.csv(feature_filename))
  gbm_predict = predict(gbm_model, image_features, n.trees = param_trees)
